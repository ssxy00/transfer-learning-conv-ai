# -*- coding: utf-8 -*-
# @Time        : 2022/10/27 22:40
# @Author      : ssxy00
# @File        : evaluate_without_type_length_limit_gpt2.py
# @Description :
...
from train_without_mc_without_type_length_limit import SPECIAL_TOKENS, pad_dataset
...
def sample_sequence(personality, history, tgt, tokenizer, model, args, current_output=None):
    ...
    for i in range(args.max_length):
        ...
        instance = build_input_from_segments(personality, history, current_output, tokenizer, max_input_len, with_eos=False)
...
def build_input_from_segments(persona, history, reply, tokenizer, max_input_len, lm_labels=False, with_eos=True):
    ...
    sequence = [seq + [eos] for seq in persona] + [seq + [eos] for seq in history] + [reply + ([eos] if with_eos else [])]
    instance = {}
...
def get_data_loaders(args, tokenizer):
    ...
    for dialog in raw_dataset:
        ...
        for utterance in dialog["utterances"]:
            ...
            lm_labels = True
            instance = build_input_from_segments(persona, history, tgt, tokenizer, args.max_input_len, lm_labels)
...
def run():
    ...
    parser.add_argument("--optimal_step", type=int, help="optimal checkpoint to load")
        ...
        with open(args.save_result_path, "w") as fout:
            for dialog in tqdm(raw_test_dataset):
                ...
                for utterance in dialog["utterances"]:
                    history = utterance["history"][-(2 * args.max_history + 1):]
                    tgt = utterance["candidates"][0]
                    out_ids = sample_sequence(persona, history, tgt, tokenizer, model, args)
                    context_strings = []
