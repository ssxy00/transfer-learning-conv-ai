# -*- coding: utf-8 -*-
# @Time        : 2022/10/27 14:15
# @Author      : ssxy00
# @File        : evaluate_without_persona_without_type_length_limit_gpt2.py
# @Description :
...
from train_without_persona_without_mc_without_type_length_limit import SPECIAL_TOKENS, pad_dataset
...
def sample_sequence(history, tgt, tokenizer, model, args, current_output=None):
    special_tokens_ids = tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS)
    if current_output is None:
        current_output = []

    for i in range(args.max_length):
        max_input_len = max(args.max_input_len - len(tgt) - 1, 1) + len(current_output)
        instance = build_input_from_segments(history, current_output, tokenizer, max_input_len, with_eos=False)
...
def build_input_from_segments(history, reply, tokenizer, max_input_len, lm_labels=False, with_eos=True):
    ...
    max_input_len = max(max_input_len, len(reply) + 1)
...
def get_data_loaders(args, tokenizer):
    ...
    for dialog in raw_dataset:
        for utterance in dialog["utterances"]:
            ...
            lm_labels = True
            instance = build_input_from_segments(history, tgt, tokenizer, args.max_input_len, lm_labels)
...
def run():
    ...
    parser.add_argument("--dataset_path", type=str,
                        default="/home1/sxy/transfer-learning-conv-ai/datasets/multiwoz/multiwoz_test.json",
                        help="Path or url of the dataset. If empty download from S3.")
    parser.add_argument("--dataset_cache", type=str,
                        default='/home1/sxy/transfer-learning-conv-ai/datasets/multiwoz/multiwoz_test.cache',
                        help="Path or url of the dataset cache")
    parser.add_argument("--model_checkpoint", type=str,
                        default="/home1/sxy/transfer-learning-conv-ai/runs/multiwoz_without_mc_without_type_length_limit/3e-4/Oct26_15-23-49_IW4202/",
                        help="Path, url or short name of the model")
    parser.add_argument("--optimal_step", type=int, default=9115, help="optimal checkpoint to load")
        ...
        with open(args.save_result_path, "w") as fout:
            for dialog in tqdm(raw_test_dataset):
                for utterance in dialog["utterances"]:
                    history = utterance["history"][-(2 * args.max_history + 1):]
                    tgt = utterance["candidates"][0]
                    out_ids = sample_sequence(history, tgt, tokenizer, model, args)
                    context_strings = []
